...
pub const LIBC: &[(&str, *const u8)] = &[
    (
        "fn write(fd: i32, buf: Ptr(u8), size: usize) isize",
        libc::write as *const u8,
    ),
    ...
    (
        "fn system(null_terminated_cmd: Ptr(u8)) i32",
        libc::system as *const u8,
    ),
];
...
pub const COMPILER: &[(&str, *const u8)] = &[
    ...
    (
        "fn Unique(Backing: Type) Type",
        Program::unique_ty as *const u8,
    ),
    (
        "fn tag_value(E: Type, case_name: Symbol) i64",
        tag_value as *const u8,
    ),
    (
        "fn tag_symbol(E: Type, tag_value: i64) Symbol",
        tag_symbol as *const u8,
    ),
    ("fn assert_eq(_: i64, __: i64) Unit", assert_eq as *const u8),
    (
        "fn assert_eq(_: Type, __: Type) Unit",
        assert_eq as *const u8,
    ),
    (
        "fn assert_eq(_: bool, __: bool) Unit",
        assert_eq as *const u8,
    ),
];
...
pub fn get_special_functions() -> String {
    ...
    writeln!(
        out,
        "//! IMPORTANT: don't try to save @comptime_addr('ASLR junk'), it won't go well. \n"
    )
    .unwrap();
    ...
    for (sig, ptr) in LIBC {
        writeln!(
            out,
            "@comptime_addr({}) @dyn_link @c_call {sig};",
            *ptr as usize
        )
        .unwrap();
    }
