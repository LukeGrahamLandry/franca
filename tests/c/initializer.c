...
int main() {
  ...
  ASSERT(0, strcmp(g22+3, "foobar"));  // clang disagrees
  ...
  ASSERT(3, ({ int x[2][3]={1,2,3,4,5,6,[0]={7,8},9,10}; x[0][2]; }));  // clang disagrees
  ...
  ASSERT(0, ({ typedef struct { int a,b; } T; T x={1,2}; T y[]={x, [0].b=3}; y[0].a; }));  // clang disagrees
  ...
  ASSERT(3, ({ typedef union { int a; struct { int b, c; }; } S; S s = (S) { .b = 1, .c =2, }; s.b + s.c; }));  // anon fields so multiple designators
